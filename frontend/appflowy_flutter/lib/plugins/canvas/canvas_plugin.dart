...
import 'package:appflowy/plugins/canvas/presentation/canvas_page.dart';
...
/// A canvas opened as a page of its own.
///
/// It reports itself as a document plugin because that is the layout it wears;
/// what makes it a canvas is the envelope on the view, which `view_ext` reads
/// before it falls through to the document page.
class CanvasPlugin extends Plugin {
  CanvasPlugin({required ViewPB view})
  ...
  @override
  PluginWidgetBuilder get widgetBuilder => CanvasPluginWidgetBuilder(
        notifier: notifier,
...
class CanvasPluginWidgetBuilder extends PluginWidgetBuilder
    with NavigationItem {
  CanvasPluginWidgetBuilder({
  ...
  /// A canvas is edge to edge: it is a space, not a document on a measure.
  @override
  ...
  Widget buildWidget({
    ...
    return BlocProvider<PageAccessLevelBloc>.value(
      ...
      child: BlocBuilder<PageAccessLevelBloc, PageAccessLevelState>(
        builder: (blocContext, state) => CanvasPage(
  ...
  @override
  String? get viewName =>
      view.name.isNotEmpty ? view.name : LocaleKeys.canvas_untitled.tr();
